...
`[features.test.coverage].lcov_paths` の最初に存在する `lcov.info` から parse される、ソースファイル単位の行カバレッジ率です。Finding は `< 100%` のファイルにのみ発行されます。Calibration は **反転値**(`100 - coverage_pct`)を保存するので、他のメトリクスと同じ「value が p95 に達したら Critical」のカスケードがそのまま使えます — フロアの調整は [Test › 設定](/heal/ja/test/configuration/#calibrationseverity-基準の調整) を参照。
