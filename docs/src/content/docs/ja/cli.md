...
| `heal logs`      | 生のフックイベントログ（`.heal/logs/`）を流し読みする。                                    |
| `heal snapshots` | メトリクス／calibrate イベント（`.heal/snapshots/`）を流し読みする。                       |
| `heal checks`    | `.heal/checks/` 内の `CheckRecord` を新しい順に一覧表示。                                  |
| `heal check`     | `.heal/checks/latest.json` のキャッシュを描画する。`--refresh` で再スキャン上書き。        |
| `heal fix`       | 修正トラッキング操作 — `show <id>` / `diff` / `mark --finding-id … --commit-sha …`。       |
...
heal fix diff                         # 直近 2 件のレコード
heal fix diff <from> <to>             # 明示ペア
heal fix diff --worktree              # ライブツリー vs 最新キャッシュ、書き込みなし
...
- **`heal check` が標準ワークフローです。** 意味のあるコミットの後
  ...
  ます。
- **`heal fix diff --worktree`** はセッション中の進捗確認に便利
