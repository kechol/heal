...
stdout がターミナルのとき、`heal status` は `$PAGER`（または `less`）にパイプします — `git diff` / `git log` と同じ慣習です。`--no-pager` を渡すか、出力をどこかへパイプ（リダイレクト、`| cat`、CI ログ）すればページャは自動的にスキップされます。`--json` は常に raw のまま stdout に書きます。
...
ライブ worktree と、指定した過去コミットでの finding を比較します。デフォルト ref は calibration の基準 SHA（`heal init` / `heal calibrate --force` が記録した `meta.calibrated_at_sha`）。記録されていないときは `HEAD` にフォールバックします — 「Progress: N% complete」が「calibration からどれだけ drain したか」として自然に読めるようにしています。
...
stdout がターミナルのとき、`heal diff` は `$PAGER`（または `less`）にパイプします — `heal status` と同じ流儀です。`--no-pager` を渡すと stdout に直接書き出します。`--json` は常にそのまま stdout に出力されます。
...
stdout がターミナルのとき、`heal metrics` も `$PAGER`（または `less`）にパイプします — `heal status` / `heal diff` と同じです。`--no-pager` で stdout に直接書き出せます。
