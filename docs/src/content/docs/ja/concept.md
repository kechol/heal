...
だからこそ Code ファミリの中で **最初に見るべきは Hotspot** です。`Critical 🔥` の項目は `Critical` の 2 倍悪いという話ではなく、その work tier の中で先に検討する項目です。`/heal-code-patch` スキルがデフォルトで `🔥` キューを最初に解消するのも同じ理由です。
