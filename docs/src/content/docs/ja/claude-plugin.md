---
title: Claude プラグイン
description: 同梱の Claude Code プラグインが、heal のメトリクスをどのように Claude セッションに繋ぐか — `/heal-code-check` 監査と `/heal-code-fix` 修復ループを含む。
---
...
- リードオンリースキル `/heal-code-check` が
  `.heal/checks/latest.json` を監査し、アーキテクチャレベルの所見と
  優先度付きのリファクタ TODO リストを返します。
- write スキル `/heal-code-fix` が同じキャッシュを Severity 順に
...
修復ループは SessionStart ナッジではなく `heal-code-fix` スキル
（後述）を通じて動きます。

## 監査スキル: `/heal-code-check`
...
## write スキル: `/heal-code-fix`

`/heal-code-fix` は `/heal-code-check` の修復ループ対応版です。
