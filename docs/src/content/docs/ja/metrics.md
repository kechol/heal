...
を、同じ生の値で別々にトリガーし、その結果を `/heal-code-patch` が消化す
る TODO リストとして提示します。
...
すべての Finding は次の 4 段階のいずれかに収まります。判定はこの順
で評価されます。

| Tier     | ルール                                                                                              |
| -------- | --------------------------------------------------------------------------------------------------- |
| Critical | `value ≥ floor_critical`（文献アンカー — 一律に悪いコードベースの逃げ道）。                          |
| Ok       | `value < floor_ok`（文献アンカー — graduation gate、**proxy メトリクスのみ**）。                     |
| Critical | `value ≥ p95`（calibrate された 95 パーセンタイル）。                                                |
| High     | `value ≥ p90`                                                                                       |
| Medium   | `value ≥ p75`                                                                                       |
| Ok       | それ以外                                                                                            |

絶対フロアがパーセンタイル分類器を二段で挟みます。

- `floor_critical` は上側の逃げ道 — これを超える値は一律に悪いコー
  ドベースでも Critical のまま。デフォルトは McCabe / SonarQube 文
  献から: CCN 25、Cognitive 50、Duplication 30%。
- `floor_ok` は下側の卒業ゲート — これ未満の値はパーセンタイルに関
  係なく Ok 固定。クリーンになったコードベースが「上位 10% は永遠に
  赤」のループ（Goodhart's Law）に縛られないようにする仕組みです。
  デフォルトは CCN 11（McCabe "simple, low risk"）、Cognitive 8
  （Sonar）。proxy メトリクスにのみ適用 — duplication / change_coupling
  / lcom はスキャン時フィルターが同じ役割を果たします。

パーセンタイル区切り（`p75 / p90 / p95`）は calibrate 時のコードベー
ス自身の分布から計算され、`.heal/calibration.toml` に保存されます。
両フロアは metric ごとに config で上書き可能です — [設定 › Floors](/heal/ja/configuration/#floors)
を参照。
...
`heal check` は非 Ok の Finding を `[policy.drain]` config 駆動で
3 段階の drain tier に振り分けます。

- **T0 — Drain queue**（デフォルト `["critical:hotspot"]`）。
  `/heal-code-patch` が drain する must-fix リスト。
- **T1 — Should drain**（デフォルト `["critical", "high:hotspot"]`）。
  余裕があれば対処、別セクションに表示、自動 drain はしない。
- **Advisory** — それ以外の非 Ok。`--all` 指定時のみ表示。

この分離が「drain to zero」を意味のあるものにします。T0 がゴール、
T1 が衛生、Advisory は余裕のあるときの review。proxy である CCN が
T0 に入るのは hotspot で裏付けられたときのみ — そうでないと
Goodhart ループに陥ります。詳細は[設定 › Drain ポリシー](/heal/ja/configuration/#drain-ポリシー)
を参照。

## CCN と Cognitive はなぜ *proxy* であって目的ではないのか

McCabe (1976) は CCN を「ブランチカバレッジに必要なテストケース数の
下限を見積もる静的指標」として導入しました — コード品質そのものを
定義するためではありません。Sonar の Cognitive Complexity (2017) も
可読性の代理指標です。これらをゼロに向けて drain しようとすると可
読性が損なわれます。

- 手続き的に凝集した関数に Extract Function を当てると、CCN は新ヘ
  ルパーへ relocate するだけでグローバル合計は減らない。
- フラットな positive composite（`if (A && B && C)`）を否定 guard
  チェーンに変換しても、元がネストしていなければ Cognitive は下が
  らず、読み手の負荷はむしろ増える。

heal の設計はこれを受け入れています。`floor_ok` で proxy メトリク
スの卒業を許容し、hotspot で「触られている」ファイルへのレバレッジ
を増幅し、drain tier モデルで TODO を「proxy と本質的問題が一致す
る Finding」に絞ります。詳細は `heal-code-review` skill の
`architecture.md` §6 のトラップカタログを参照。
...
`floor_ok` 未満の関数はパーセンタイルに関係なく Ok 固定です。これに
よりクリーンになったコードベースは proxy メトリクスから卒業でき、
パーセンタイル分類器による「常に上位 10% を flag」ループ（Goodhart's
Law）から抜けられます。
