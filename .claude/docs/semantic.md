...
| `doc_pairs` (on demand) | docs | unpaired docs → `noul` per candidate source (≤20, ranked by path / heading word overlap): "does the page document this file?". A single `choice` answered `none` on every page here, since pages document several files | `report`: `pairs[] {doc, srcs, confidence, scores, source: "llm"}` with every source ≥ cutoff 0.6, `confidence` = the lowest; `/heal:setup docs` writes them as `PairSource::Llm` (no Jev variant: older binaries parse `source` strictly) |
