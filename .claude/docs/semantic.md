...
| `term_drift` | code | depends on `concept`. Per concept, frequent non-verb words of function names (≥2 names, top 8) that never co-occur → `noul` "same thing?" | `term_drift` (p ≥ 0.7; location = first file of the minority word, `locations` = its other files) |
