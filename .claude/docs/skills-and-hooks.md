...
| `heal-doc-patch` | `[features.docs]` | Drain doc findings one finding per commit (broken internal links, dangling identifier removal, orphan registration, resolvable TODOs). Same three-branch loop as `heal-code-patch`: allow-list (apply) / false-positive (propose `heal mark accept`) / escalate-list (stop). Refuses dirty worktree. | write counterpart of `heal-doc-review` |
| `heal-test-reporter-setup` | `[features.test]` | Detect language stack (Rust / Python / JS-TS / Go / Scala / mixed), then with per-step `AskUserQuestion` approval install the lcov reporter, flip `[features.test.coverage].enabled`, run the reporter, and optionally wire `[features.test.coverage].post_commit_refresh` so HEAL's post-commit hook re-runs the reporter detached after every commit. CI workflow edits stay a copy-pasteable proposal. | — |
...
| `heal-test-patch` | `[features.test]` | Drain test findings one finding per commit (writing missing unit tests for uncovered hotspots, aligning drifted tests, re-enabling skipped tests whose reason no longer holds). Same three-branch loop as `heal-code-patch`: allow-list (apply) / false-positive (propose `heal mark accept`) / escalate-list (stop). Runs the test suite per commit; refuses to weaken assertions or skip flakes. | write counterpart of `heal-test-review` |
