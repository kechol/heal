...
impl ChurnObserver {
    ...
    pub(crate) fn scan_history(&self, history: &History) -> ChurnReport {
        ...
        for commit in &history.commits {
            let contributed = !commit.paths.is_empty();
            ...
            for (path, added, deleted) in &commit.line_stats {
                let entry = per_file
                    .entry(path.clone())
                    .or_insert_with(|| FileChurn::new(path.clone()));
                entry.lines_added = entry.lines_added.saturating_add(*added);
                entry.lines_deleted = entry.lines_deleted.saturating_add(*deleted);
            }
