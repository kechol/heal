...
///
/// Code Hotspot is a single-axis composite (Tornhill's "true risk =
/// volatility × complexity"). Test and Docs have their own per-family
/// composites — `test_hotspot` and `doc_hotspot` — and decorate
/// findings within their own families. Mixing signals here would
/// re-introduce the "rank by feature count" failure mode that earlier
/// boost variants tried to cap.
#[must_use]
pub fn compose(
    churn: &ChurnReport,
