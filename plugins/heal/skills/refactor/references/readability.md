...
Reference loaded by `/heal:refactor` when evaluating whether a proposed
...
## 3. Heuristics for judgment in /heal:refactor
...
but worth surfacing during exploration. `/heal:refactor` should
...
## How `/heal:refactor` uses this reference

When proposing a refactor in the Propose step:

1. After identifying a candidate via the metric and the triage
   taxonomy, run the **5-question judgment test** in §3.
2. If all five answers are clearly "yes", make it a numbered
   proposal.
3. If any answer is "no" or "unsure", hold it as a *deferred
   question* — present the trade-off in the deferred-questions
   section rather than as a proposal to apply.
