...
Reference loaded by `/heal:refactor` when proposing refactors. The
...
`/heal:refactor` proposes them with Risk `high`, marks them **needs a
decision**, and applies one only after the user picks the concrete
alternative (which boundary, which name, which side owns what).
