...
`/heal:setup` when it needs to justify a setting, and by
`/heal:refactor` when a finding's "fix" is to relax a threshold.

The file lives at `<project>/.heal/config.toml`. It is parsed by
heal's `Config::load`, which is **strict** — `deny_unknown_fields`
...
- **T0 (`must`)** — drain to zero. `/heal:refactor` proposes from T0 first.
...
`/heal:setup` picks per-strictness values from this table.
...
- The drain promotion in Strict (`high:hotspot` joins `must`) is the
  single biggest behavioral change — it makes `/heal:refactor` treat
  hotspot-flagged High findings as if they were Critical.
...
`/heal:setup` checks for this fit before offering Strict
