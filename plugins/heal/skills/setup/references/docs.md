# Docs family: enable and pair

Loaded by `/heal:setup` for the docs step. Part 1 turns
`[features.docs]` on with document globs fitted to this repository.
Part 2 builds `.heal/doc_pairs.json`, the doc ⇔ src map the docs
observers read. Skip Part 1 when doctor reports the family enabled;
run Part 2 whenever doctor reports the pairs file missing, stale, or
naming paths that no longer exist.

## Part 1 — Enable `[features.docs]`
...
useful. Part 1 writes the toggle and the standalone globs; Part 2
generates `doc_pairs.json`.
...
2. **Ask once** with `AskUserQuestion`:
   ...
   Options:
     - "Enable": flip [features.docs].enabled = true, populate
                 ...
                 layout, then build the doc pairs (Part 2).
...
4. **Continue with Part 2** so `.heal/doc_pairs.json` lands before
   the next `heal status` run. Say so before starting, so the user
   can stop and edit the standalone globs first.

## Part 2 — Doc pairs (`.heal/doc_pairs.json`)
...
The file format is in `doc-pairs-schema.md` next to this file.
