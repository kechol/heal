# Test family: enable and wire coverage

Loaded by `/heal:setup` for the tests step. Part 1 turns
`[features.test]` on with `test_paths` and `lcov_paths` fitted to this
repository. Part 2 installs and runs a coverage reporter so heal can
read `lcov.info`. Skip Part 1 when doctor reports the family enabled;
run Part 2 when coverage is wanted and doctor finds no lcov file.

## Part 1 — Enable `[features.test]`

The test family runs the `coverage_pct` and `skip_ratio` observers
plus tags every Finding's `is_test_file` flag against
`test_paths`. Part 1 writes the toggle, `test_paths`, and
`lcov_paths`; the language-specific lcov reporter is Part 2.
...
2. **Ask once** with `AskUserQuestion`:
   ...
   Options:
     - "Enable": flip [features.test].enabled = true, populate
                 test_paths and lcov_paths from the survey, then wire
                 a coverage reporter (Part 2).
...
4. **Continue with Part 2** so the language-specific reporter
   (cargo-llvm-cov, pytest-cov, vitest / jest, gcov2lcov,
   scoverage) lands. Part 2 installs the reporter, flips
   `[features.test.coverage].enabled = true`, runs the reporter,
   and verifies — each step gated by its own `AskUserQuestion`.
   Say so before starting so the user can stop here. CI workflow
   edits stay a proposal; setup never edits CI on its own.

## Part 2 — Coverage reporter
...
1. **`[features.test]` enabled.** Read `.heal/config.toml`. If
   ...
   declined — the rest of this part has no consumer.
...
> - **Skip**: leave the reporter to the user; this step stops here.
...
On Skip: stop this step. Re-invoking later resumes from this step
...
> - **Skip**: keep the current config; this step stops here.
...
Re-running this step after Apply detects the existing
...
Next:
  /heal:tests   # read the test findings, propose, and apply
```
...
- **Output language** follows `references/cli.md` in the plugin
  root; CI and config files keep their own language.
