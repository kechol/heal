...
Loaded by `/heal:docs` in scaffold mode. Produces (or extends) the project's documentation
...
scaffold mode is the only producer.

This is the **bootstrap** side of doc maintenance. The review and fix
side of `/heal:docs` works on pages that exist; it cannot stand up a
tree from nothing — that's what this mode is for.
...
- After enabling `[features.docs]` and building the doc pairs with
  `/heal:setup`, the team realises half the pairs point at pages
  that don't exist yet — scaffold them so `doc_coverage` findings
  flow.

To **review** or **fix** existing docs, use the default mode of
`/heal:docs`. To **map** docs ↔ src, use `/heal:setup docs`.
...
Doc scaffold (reconcile mode):
  ...
  next steps:
    ...
    - Run /heal:setup docs to register the new pages in doc_pairs.json.
